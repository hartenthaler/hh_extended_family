...
![Downloads](https://img.shields.io/github/downloads/hartenthaler/hh_extended_family/total?label=downloads)
...
Person-level relationship distances use the [WikiTree Degree concept](https://www.wikitree.com/wiki/Help:Degrees): parents, children, partners, and siblings are each one Degree away. Optional gold-colored Degree labels show each person's shortcut-free paths in a tooltip. The summary can additionally show Degree shells from Degree 0 (the proband) onward, with linked people grouped by path. This Degree table is placed directly below the direct-line table and both captions are visually separated from the following content. These display options do not change which people belong to the extended family. If additional partner-chain people are configured not to count as members, their Degree labels remain visible in the partner chains but they are excluded from the Degree shells, loop detection, and clippings-cart export. Technical details are documented in `docs/architecture.md`.
